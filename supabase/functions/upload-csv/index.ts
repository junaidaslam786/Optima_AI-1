...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    const upRes = await fetch(`${SUPABASE_URL}/storage/v1/object/${BUCKET}/${encodeURIComponent(objPath)}`, {
      method: "PUT",
      headers: {
    ...
    for (const row of rows){
      ...
      if (panelCache.has(panelName)) {
        ...
        const lookup = await fetch(`${SUPABASE_URL}/rest/v1/panels?select=id&name=eq.${q}`, {
          headers: {
            "Authorization": `Bearer ${SERVICE_ROLE_KEY}`,
            "apikey": SERVICE_ROLE_KEY
          }
