...
deno: Deno.serve(async (req) => {
  ...
  if (!user_id || !panel_id) {
    return new Response(
      JSON.stringify({ error: "Missing user_id or panel_id" }),
      { status: 400, headers: CORS }
    );
  }
  ...
  const markersRes = await fetch(
    `${SUPABASE_URL}/rest/v1/markers?user_id=eq.${encodeURIComponent(user_id)}&panel_id=eq.${encodeURIComponent(panel_id)}&select=marker,value,unit,status,panel:panels(name)\`,
    {
      headers: {
  ...
  if (!markersRes.ok) {
    ...
    return new Response(
      JSON.stringify({ error: "Failed to fetch markers" }),
